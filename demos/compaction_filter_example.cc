...
class MyFilter : public rocksdb::CompactionFilter {
 public:
  ...
  bool FilterMergeOperand(int level, const rocksdb::Slice& key,
    ...
    fprintf(stderr, "FilterMerge(%s, %s)\n", key.ToString().c_str(), existing_value.ToString().c_str());
    ++merge_count_;
...
int main() {
  ...
  system("rm -rf ./compactionfilterexample");
  rocksdb::Options options;
  options.create_if_missing = true;
  options.merge_operator.reset(new MyMerge);
  options.compaction_filter = &filter;
  status = rocksdb::DB::Open(options, "./compactionfilterexample", &raw_db);
  assert(status.ok());
